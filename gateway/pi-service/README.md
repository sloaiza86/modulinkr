...
| `set_ai.sh` | Acción privilegiada de la página "Asistente de IA" del visor. Reescribe una lista cerrada de claves `MODULINKR_AI_*` de `web.env` por stdin, conserva la credencial cuando se omite y no imprime sus valores. |
| `set_wifi.sh` | Acción privilegiada de la página "Configurar red WiFi" del visor. Con NetworkManager (`nmcli`): `scan` lista las redes visibles (rescan) y `connect` conecta a un SSID leyendo la contraseña de stdin (`nmcli --ask`, nunca por argumentos). NetworkManager guarda el perfil; el script no escribe la contraseña en ningún archivo. |
...
| `get_net.sh` | Lectura privilegiada (solo lectura) de los parámetros de red de `gateway.env` que todo nodo debe compartir para unirse (región, frecuencia, Network ID, TTL, SF, ancho de banda, seguridad y el broker MQTT, incluida la clave). El asistente los usa para bloquear esos campos del formulario a los valores reales. Extrae con `sed` para tolerar claves ausentes. |
| `set_net.sh` | Acción privilegiada de la página "Parámetros de red LoRa" del visor (camino B). Reescribe por stdin las claves LoRa de `gateway.env` (región, frecuencia, Network ID, SF, ancho de banda, TTL, seguridad) con lista blanca y reinicia el servicio del gateway, que al arrancar reempuja los parámetros de radio al Heltec (comando `RADIO`, `frame-format.md` §12.6), así que un cambio de red se aplica en caliente sin reflashear. Solo toca las claves recibidas. |
...
| `MODULINKR_SF` | `7` | SF del despliegue: ToA del aire propio (v3.1) y empuje `RADIO` al Heltec (camino B) |
| `MODULINKR_BW_KHZ` | `125` | Ancho de banda LoRa, kHz (camino B: se empuja al Heltec con `RADIO`) |
