...
Documento normativo del **formato JSON** que el supernodo publica vía MQTT cuando el respaldo selectivo NB-IoT se activa. Cada mensaje publicado contiene un batch con las muestras que no fueron confirmadas por LoRa.
...
Tres escenarios disparan la publicación de un batch:

| Disparador | Origen | Contenido |
| --- | --- | --- |
| `"failover"` | Automático. Se cumplió la condición de `nbiot.failover_missed_acks` en `nbiot.failover_window_ms` (ver `node-config.md` §4.3). | Las muestras correspondientes a las tramas LoRa **no confirmadas** que están en la cola del nodo. |
| `"manual"` | Comando externo (`commands-format.md`): el operador o el backend ordena vaciar la cola por NB-IoT inmediatamente. | Igual que `failover`: las no confirmadas en cola. |
...
| `node_id` | integer | sí | Coincide con `node.id` del config. Rango 1-254. |
...
| `trigger` | string | sí | Uno de `"failover"`, `"manual"`, `"test"`. Ver §1. |
...
Cada elemento del array `samples` representa **una muestra capturada en un instante** que no fue confirmada por LoRa.
...
| Campo | Tipo | Obligatorio | Notas |
| --- | --- | --- | --- |
| `seq` | integer | sí | `seq` original (uint16) de la trama LoRa que llevaba esta muestra y que no fue confirmada. Permite al backend evitar duplicar muestras que sí llegaron por LoRa más tarde. |
| `ts` | integer | sí si `clock_synced == true` | Timestamp Unix epoch en segundos (UTC). Si `clock_synced == false`, puede aparecer `null` o un valor best-effort. |
| `v` | array de floats | sí | Valores de cada `read[]` en el mismo orden que el config del nodo (igual que en `frame-format.md` §2.1). Cada elemento es el valor ya convertido a su unidad real (edge computing). |

### 4.1 Orden de las muestras

Las muestras dentro de `samples` van ordenadas por `seq` ascendente, respetando el wraparound modular descrito en `frame-format.md` §4.4. Si una racha cruza el wraparound, las muestras post-wrap (`seq` pequeño) aparecen después de las pre-wrap (`seq` grande).
...
- Tope práctico: el tamaño de la cola de tramas pendientes del nodo (recomendado 256 entradas, ver `frame-format.md` §4.1).
...
```json
{
  ...
  "fw_version":     "0.0.2-h1",
  "samples": [
    { "seq": 1431, "ts": 1718000010, "v": [24.5, 51.2] },
    { "seq": 1432, "ts": 1718000011, "v": [24.5, 51.3] },
    { "seq": 1434, "ts": 1718000013, "v": [24.6, 51.2] },
    { "seq": 1435, "ts": 1718000014, "v": [24.6, 51.1] },
    { "seq": 1437, "ts": 1718000016, "v": [24.7, 51.0] }
  ]
}
```
...
- El backend, al recibir esto, sabe que tiene `seq` 1431, 1432, 1434, 1435, 1437 de la fuente NB-IoT. Si en algún momento llega un ACK retrasado de LoRa para 1431, simplemente se ignora (idempotencia por `seq` + `node_id`).
- Tamaño total del payload MQTT: ~330 bytes JSON. Más overhead MQTT (~50 bytes con TLS) = ~380 bytes. Comodidad para SIM IoT.

### 7.1 Ejemplo de batch de test (vacío)
...
Sirve para verificar que la cadena NB-IoT → broker funciona sin necesidad de haber perdido tramas.
...
3. `trigger` fuera del enum `{"failover", "manual", "test"}`.
4. `samples[i].seq` fuera del rango uint16 (`0` a `65535`).
5. `samples[i].v` con un número de elementos distinto al `len(reads[])` del config del nodo emisor.
6. Para `trigger != "test"`, `samples` vacío es inválido (indica un error en el firmware al construir el batch).
7. Si `clock_synced == true`, todos los `samples[i].ts` deben estar presentes y no ser `null`. Si falta alguno, el batch es inválido.
...
| --- | --- | --- | --- |
| Failover corto (5 muestras) | 5 | ~330 B | ~400 B |
| Failover medio (30 muestras) | 30 | ~1.8 KB | ~1.9 KB |
| Failover largo (256 muestras, cola al límite) | 256 | ~15 KB | ~15.2 KB |
