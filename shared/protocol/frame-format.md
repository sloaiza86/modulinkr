...
`nb_flags` es un mapa de bits: bit 0 (`0x01`) el registro celular está confirmado por una consulta reciente, bit 1 (`0x02`) la sesión MQTT con el broker cloud está operativa, bit 2 (`0x04`) el estado MQTT es desconocido y bit 3 (`0x08`) el registro celular es desconocido. Los bits 0 y 3 son excluyentes, igual que los bits 1 y 2. El registro no se deduce del estado interno `READY`: se conserva el resultado de `AT+CEREG?` y la hora de esa comprobación. `csq` es la calidad de señal cruda 0 a 31 (`0xFF` si es desconocida), la misma escala del `SN_OFFER` (§8.2).

La consulta local `AT+CMQTTDISC?` se programa cada 60 s en la tarea del módem mientras está en estado `READY`, aunque no haya muestras pendientes. Una respuesta válida con `disc_state=0` confirma conexión y con `disc_state=1` confirma desconexión. En este último caso se reinicia la sesión MQTT. Una respuesta inválida o incompleta activa el bit 2 y se vuelve a consultar al siguiente intervalo. La confirmación inicial de conexión también activa el bit 1. Una comprobación positiva con más de 180 s de antigüedad pasa a desconocida para no perpetuar un estado si la tarea deja de avanzar. Las operaciones AT en curso pueden retrasar el sondeo; no se bloquea el procesamiento LoRa. El keepalive MQTT se mantiene en 300 s y no se añaden publicaciones de prueba.

El registro celular también se consulta cada 60 s, incluso durante los intentos de conexión MQTT, mediante `AT+CEREG?` por UART. El registro inicial usa la misma consulta con la cadencia de 5 s del estado `REGISTERING`. Se aceptan como registrado los estados de red propia y roaming, solo con una respuesta completa. Una respuesta válida sin registro devuelve el servicio a `REGISTERING`; una respuesta ausente o inválida marca NB-IoT y MQTT como desconocidos y reinicia la inicialización tras el backoff de 30 s. El registro caduca a los 180 s desde la consulta real, aunque sigan llegando heartbeats LoRa. Una caída exclusiva de MQTT conserva NB-IoT conectado mientras el registro celular siga confirmado. La inicialización de la UART invalida los estados anteriores.

Los bits 2 y 3 se incorporan de forma aditiva el 2026-09-05, sin cambiar la longitud del heartbeat ni el significado de los bits 0 y 1. Un visor anterior no distingue desconocido de desconectado. La consulta refleja el estado conocido por el módem; una pérdida silenciosa del enlace puede detectarse después, mediante el mantenimiento de la sesión MQTT. La semántica de `disc_state` está definida en el [manual MQTT(S) del SIM7028, §3.2.9](https://files.waveshare.com/wiki/SIM7028-NB-IoT-HAT/SIM7028%20Series_MQTT%28S%29_Application%20Note_V1.03.pdf). Queda pendiente comprobar la respuesta del módem y los tiempos de detección en el banco.
