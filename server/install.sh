...
# el broker MQTT (Mosquitto con TLS) y la base de datos (PostgreSQL). Pensado
# para una VM Ubuntu de poca memoria y para ser reejecutable sin romper una
# instalación previa.
#
# Uso:
...
#
# Componentes: broker | database | all
set -euo pipefail
...
menu() {
    ...
    echo "  3) all       broker y database"
    local choice; read -r -p "Opción [3]: " choice; choice="${choice:-3}"
    case "$choice" in
        1) COMPONENTS="broker" ;;
...
main() {
    ...
    local want_broker=0 want_db=0
    case ",$COMPONENTS," in
        *,all,*)      want_broker=1; want_db=1 ;;
        *) case ",$COMPONENTS," in *,broker,*) want_broker=1 ;; esac
           case ",$COMPONENTS," in *,database,*) want_db=1 ;; esac ;;
    esac
    [ "$want_broker" = 0 ] && [ "$want_db" = 0 ] && die "Nada que instalar: componentes '$COMPONENTS'."
